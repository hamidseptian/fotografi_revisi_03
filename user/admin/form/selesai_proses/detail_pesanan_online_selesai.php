...
left join harga_edit as c on a.id_he = c.id_he 
	where a.status not in ('Masuk Ke Keranjang') and a.id_pelanggan='$id' and a.waktu_pesan='$waktu'");
$j = mysqli_num_rows($q);
...
left join paket as b on a.id_paket=b.id_paket
	where a.status not in ('Masuk Ke Keranjang') and a.id_pelanggan='$id' and a.tanggal_booking='$waktu'");
		...
		<tr>
			...
			<td>
				<?php if ($d['status']=='Selesai') { ?>
					<a href="form/selesai_proses/ambil_foto.php?id_cetak=<?php echo $d['id_cetak'] ?>&idp=<?php echo $id ?>&waktu=<?php echo $waktu ?>&via=<?php echo $via ?>&id_tagihan=<?php echo $id_tagihan ?>&menu=selesai_cetak" class="btn btn-info btn-xs" onclick="return confirm('Apakah anda yakin.?')">Sudah di ambil pelanggan</a>
				<?php } ?>
			</td>
...
echo number_format($total);
			...
			<td width='30%'><?php echo $d_book['status'] ?></td>
			<td>
				<?php if ($d_book['status']=='Selesai') { ?>
					<a href="form/selesai_proses/ambil_paket.php?id_booking=<?php echo $d_book['id_booking'] ?>&idp=<?php echo $id ?>&waktu=<?php echo $waktu ?>&via=<?php echo $via ?>&id_tagihan=<?php echo $id_tagihan ?>&menu=selesai_cetak" class="btn btn-info btn-xs" onclick="return confirm('Apakah anda yakin.?')">Sudah di ambil pelanggan</a>
				<?php } ?>
			</td>
		</tr>
...
<form  method="post" action="form/selesai_proses/bayar.php?via=<?php echo $via ?>" enctype="multipart/form-data">
...
<?php } ?>
	...
	<?php if ($total_bayar>0): ?>
		<br>
		<a href="form/pesanan/print_tanda_terima.php?waktu=<?php echo $waktu ?>&id_tagihan=<?php echo $idtagihan ?>&id_pelanggan=<?php echo $id ?>" target="_blank" class="btn btn-info btn-xs">Print Tanda Terima</a>
	<?php endif ?>
</div>
<?php } ?>